...
## v4.2.0 (2026-03-26) — APP 下载 & 看板娘优化

### 🆕 新增功能

#### 📱 移动端 APP 下载
- 导航栏新增 **APP 下载按钮**，点击弹出下载面板
- 支持**扫码下载**，PC 端扫描二维码即可在手机上直接安装
- 提供 bilibilias 安卓客户端下载，支持更丰富的移动端功能
- APP 基于开源项目 [bilibilias](https://github.com/1250422131/bilibilias)，感谢原作者的贡献

### 🐛 修复

#### 🎭 看板娘显示优化
- 修复部分 Live2D 模型**超出容器边界**的问题
- 新增 `overflow: hidden` 兜底裁剪，防止模型溢出
- 优化 `injectLayout` 逻辑：保留模型原始布局参数，仅对缺失布局的模型补充默认值

### 📡 新增 API
| 端点 | 说明 |
|------|------|
| `GET /api/app/list` | 获取可用的 APP 安装包列表 |
| `GET /api/app/download/:filename` | 下载指定的 APK 文件 |
